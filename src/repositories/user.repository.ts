...
class UserRepository {
    ...
    public async create(dto: IUser): Promise<IUser> {
        ...
        const indexUsername = users.findIndex((user) => user.username === dto.username);
        if (indexEmail !== -1) {
    ...
    public async update(id: number,dto: IUser): Promise<IUser> {
        const users = await fsService.read();
        const user = users.find(user => user.id === id);
        const indexUsername = users.findIndex((user) => user.username === dto.username && user.id !== id);
        const indexEmail = users.findIndex((user) => user.email === dto.email && user.id !== id);
        if (!user) {
            throw new ApiError('User not found', 404)
        }
        if (indexUsername !== -1) {
