    ...
    public async update(id: number, dto: IUser): Promise<IUser> {
        ...
        if (username !== undefined) {
            if (typeof username !== 'string' || username.length < 3 || username.length > 20) {
                throw new ApiError('Missing or invalid data in the field "username". The "username" field is required, must be a string between 3 and 20 characters.', 400);
            }
        }
        if (age !== undefined) {
        ...
        if (email !== undefined) {
            if (typeof email !== 'string' || email.length < 5 || email.length > 50 || !email.includes('@')) {
                throw new ApiError(
                    'Missing or invalid data in the field "email". The "email" field must be a string between 5 and 50 characters and contain an "@" symbol.',
                    400);
            }
